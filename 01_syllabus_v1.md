...
|---|------|--------------|------|---------|
| 1 | 15/09 8h30-10h30 | CM | Formats de fichier & environnement de développement | texte vs binaire, hexadécimal (PGM), ASCII/Unicode ; IDE (dossier=projet), terminal, aperçu Markdown |
...
| 3 | 29/09 8h30-10h30 | CM | Environnement Python & construction d'une CLI | conda/conda-forge, `subprocess`, `argparse`, fichiers via `pathlib` |
...
## Cours 1 — Formats de fichier & environnement de développement (CM)

Objectif : comprendre ce qu'est un fichier (format, extension) et savoir travailler dans un IDE.

- **🎓 12′ · Formats & extensions** : texte vs binaire (docx/pdf/zip/png/vidéo vs `.txt`/`.md`/`.html`) ; renommer une extension, ouvrir avec le bon logiciel.
...
- **⌨️ 20′ · Voir un fichier binaire** : ouvrir une petite image **PGM/PPM** (Netpbm) en **hexadécimal** — en-tête lisible + octets de pixels ; ASCII (`P2`/`P3`) vs binaire (`P5`/`P6`), *même image, deux encodages*. Prépare le TD7 (une image = un tableau de pixels).
- **🎓 8′ · Qu'est-ce que programmer** (très haut niveau) : un langage = du texte suivant des conventions ; interpréteur vs compilateur ; sortie binaire ou action.
- **🎓 12′ · IDE (VSCode)** : *dossier = projet* (pas fichier isolé), explorateur, palette de commandes, terminal intégré, aperçu Markdown, extensions.
- **🎓 5′ · Fichiers & dossiers cachés** : nom commençant par `.` (`.gitignore`, dossier `.git/`) ; afficher les fichiers cachés — **prérequis** de `.gitignore` (cours 2).
- **⌨️ 25′ · Manipulation** : installer VSCode, ouvrir un dossier, créer un `.md` et le prévisualiser, lancer un petit script Python (calcul / conversion simple).
...
*Détail & exercices : [`cours/1_formats_et_environnement/`](cours/1_formats_et_environnement/).* ⚠️ **Séance dense** (~107′ de contenu + transitions) — envisager d'alléger (ex. Notebook/PGM en autonomie).
...
- **⌨️ 40′ · Manipulation guidée — dépôt de notes du cours** (chaque étudiant en parallèle du tableau) :
...
## Cours 3 — Environnement Python & construction d'une CLI (CM)

Objectif : installer un environnement reproductible et écrire un petit outil Python qui **orchestre des commandes**.

- **⌨️ 20′ · Environnement (conda / conda-forge)** : `conda create -n info01 -c conda-forge python ffmpeg imagemagick pandoc typst` puis `conda activate` ; commandes fournies et **testées** dans un README (postes labo et perso). L'env sert à *installer les outils*, pas à packager un projet.
- **🎓 12′ · Fichiers en Python avec `pathlib`** : chemins portables (Windows inclus), lecture / écriture — c'est *ici* qu'on manipule les fichiers, pas au shell ; template (optionnel, un plus).
- **🎓 13′ · Appel de commandes externes** : `subprocess.run([...], check=True)` (forme liste, codes de retour) ; enchaîner des étapes = automatisation.
...
**⌨️ ~55′ · Jalons de code = étapes git** (manipulation guidée) :
...
## TD (séance 7) — Capstone : benchmark image + rapport (PR)
...
| Séance | Dossier |
|--------|---------|
| 1 | [`cours/1_formats_et_environnement/`](cours/1_formats_et_environnement/) ✅ rempli (gabarit) |
