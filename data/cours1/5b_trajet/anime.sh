...
# fichier que l'étape suivante consomme : c'est ce que le TD montre.

set -euo pipefail
cd "$(dirname "$0")"
# Tout ce que le script fabrique naît à côté de lui, comme l'exécutable du
# hello world. Le fond de carte est à côté aussi dans l'archive remise aux
# étudiants ; dans le dépôt, carte.py l'a déposé dans produit/.
carte=carte.png
[ -f "$carte" ] || carte=produit/carte.png
