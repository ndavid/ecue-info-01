---
title: Anaconda
subtitle: Anaconda Prompt, conda, Navigator
---

## Ce qu'il faut savoir sur les postes de la salle

Anaconda est installé pour tous les utilisateurs, dans
`C:\ProgramData\anaconda3`. Un compte élève peut lire ce dossier, mais pas y
écrire. On n'installe donc rien dans l'environnement `base` : chaque TD qui
a besoin d'un paquet crée son propre environnement (TD 4a), qui va dans
`C:\Users\<nom>\.conda\envs`.

Le raccourci « Anaconda Prompt » du menu Démarrer ouvre un `cmd` et y active
`base`. Sa cible (clic droit sur le raccourci, Propriétés, champ Cible)
contient le chemin d'Anaconda. Quand une entrée ci-dessous demande ce
chemin, il se lit à cet endroit.

Navigator n'est pas nécessaire au module. VS Code se lance depuis le menu
Démarrer ([annexe VS Code](vscode.md)), et JupyterLab depuis l'Anaconda
Prompt ([annexe JupyterLab](jupyterlab.md)).
...
| Réglages de conda pour le compte | `C:\Users\<nom>\.condarc` |
...
: Cliquer une seule fois, puis attendre deux minutes, avec la session
  ...
  depuis le menu Démarrer ([annexe VS Code](vscode.md)). Une mention
...
: Employer un `cmd` à la place de PowerShell : `activate.bat` n'est pas un
  ...
  Prompt, et ce que le terminal « Anaconda Prompt » donne à VS Code
  ({ref}`V5 <dep-v5>`).
...
conda-forge, qui n'a pas de conditions à accepter.
